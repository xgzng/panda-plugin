#!/usr/bin/env node
// Every panda command the pi extension registers must also ship as a
// file-based command for the hosts that need one: Claude Code (commands/*.toml,
// which Gemini CLI reuses) and OpenCode (.opencode/command/*.md). /panda-help
...
test('pi registers at least the base command', () => {
  assert.ok(commands.includes('panda'), 'expected pi to register a panda command');
});
