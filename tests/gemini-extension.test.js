...
// always-on context, commands/*.toml for /panda + /panda-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring panda.
...
const REUSED_COMMANDS = ['commands/panda.toml', 'commands/panda-review.toml'];
const REUSED_SKILLS = ['skills/panda/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Panda's
...
test('manifest names the panda extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the panda rules', () => {
  const manifest = loadManifest();
