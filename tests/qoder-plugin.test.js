...
test('qoder hooks config exists and registers UserPromptSubmit', () => {
  ...
  assert.ok(cmd.includes('ponytail-mode-tracker.js'), 'must point at the retained upstream hook filename');
});

test('qoder rules file exists and is non-empty', () => {
  const rulesPath = path.join(root, '.qoder', 'rules', 'panda.md');
  assert.ok(fs.existsSync(rulesPath), '.qoder/rules/panda.md must exist');
  const content = fs.readFileSync(rulesPath, 'utf8').trim();
  assert.ok(content.length > 0, '.qoder/rules/panda.md must not be empty');
  assert.ok(content.includes('lazy senior developer'), 'rules must contain the panda identity');
});
...
test('qoder rules match AGENTS.md canonical body', () => {
  ...
  const qoderCopy = fs.readFileSync(path.join(root, '.qoder', 'rules', 'panda.md'), 'utf8')
    .replace(/\r\n/g, '\n').trim();
  assert.equal(qoderCopy, canonical, '.qoder/rules/panda.md drifted from AGENTS.md');
});
